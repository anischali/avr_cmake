...
int i2c_master_write_byte(struct i2c_bus_t *bus, uint8_t b)
{
    bus->twdr = b;
...
int i2c_master_transaction(struct i2c_bus_t *bus, struct i2c_packet_t *pkt)
{
    ...
    if (pkt->subaddr_len > 0) {
        status = i2c_master_write_byte(bus, pkt->slave_addr << 1);
        if (status != TW_MT_SLA_ACK)
            return 19;
        
        b = &pkt->subaddr.subaddr_buf[0];
        while ((--pkt->subaddr_len) >= 0) {
            status = i2c_master_write_byte(bus, *b++);
        ...
        if (pkt->dir == TW_READ) {
            ...
            status = i2c_master_write_byte(bus, sla);
...
read:
    ...
    while(len-- > 0) {
        status = i2c_master_read_byte(bus, b, (len == 1));
